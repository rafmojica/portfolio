...
export function TechStackSection({
  autoOpen = false,
  openSpeedMs = 70,
}: { autoOpen?: boolean; openSpeedMs?: number } = {}) {
  ...
  const openChest = useCallback(() => {
    ...
    timerRef.current = window.setInterval(() => {
      ...
      const f = Math.min(
        LAST_FRAME,
        Math.floor((Date.now() - start) / openSpeedMs) + 1,
      );
      if (f >= LAST_FRAME) {
        window.clearInterval(timerRef.current);
  ...
  const closeChest = useCallback(() => {
    ...
    timerRef.current = window.setInterval(() => {
      const f = Math.max(
        0,
        from - Math.floor((Date.now() - start) / CLOSE_SPEED_MS) - 1,
      );
      if (f <= 0) {
        window.clearInterval(timerRef.current);
        playingRef.current = false;
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        background:
          "linear-gradient(180deg, #121217 0%, #0f0f13 55%, #0b0b0e 100%)",
      ...
      <div style={{ position: "relative", zIndex: 2, textAlign: "center" }}>
        ...
        <h1
          style={{
            ...
            letterSpacing: "1px",
            background:
              "linear-gradient(180deg, #f7efd9 8%, #e2d5b0 50%, #bfae83 100%)",
            WebkitBackgroundClip: "text",
            backgroundClip: "text",
            color: "transparent",
            filter:
              "drop-shadow(0 3px 0 rgba(20,16,34,.8)) drop-shadow(0 6px 12px rgba(0,0,0,.6))",
          }}
        >
          Tech Stack
        </h1>
        <div
          style={{
            ...
            margin: "12px auto 0",
            background:
              "linear-gradient(90deg, transparent, #8b7fb8, transparent)",
          }}
        />
        <p
          style={{
                  ...
                  >
                    <img
                      ...
                      style={{
                        ...
                        animation: glowAnim
                          ? `${glowAnim} 5s ease-in-out ${i * 0.7}s infinite`
                          : "none",
                      }}
                    />
                  </div>
